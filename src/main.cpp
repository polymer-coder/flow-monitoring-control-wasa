...
#define ANALOG_OUTPUT_AIR_PRESSURE CONTROLLINO_AO0 // Hardware modded channel to get current output instead of voltage
#define ANALOG_INPUT_FLOWRATE CONTROLLINO_AI12     // this input is a special 0-10V input. The remaining ore 0-24V

// Pipe dimensions for flow meter
#define PIPE_INTERNAL_DIAMETER 0.0508      // 2" = 0.0508m
#define PIPE_INTERNAL_RADIUS 0.0254        // 1" = 0.0254m
...
BLYNK_WRITE(BLYNK_FLOWRATE_SETPOINT) // this command is listening when something is written to V1
{
  ...
  analogWrite(ANALOG_OUTPUT_AIR_PRESSURE, adcVal);
...
// Otherwise on every reset the rtc will take the time of the last time a script was uploaded
  ...
  pinMode(ANALOG_OUTPUT_AIR_PRESSURE, OUTPUT); // 4-20mA out to control valve for air supply
  pinMode(ANALOG_INPUT_FLOWRATE, INPUT);       // 0-10V input from INOR transmitter. Used to convert 4-20mA from flowmeter to 0-10V input to flow meter

  
  analogWrite(ANALOG_OUTPUT_AIR_PRESSURE, 51); // set analog output to 4mA
  ...
  // After blynk boots up, synchronize the value of the Blynk app to the Controllino
  Blynk.syncVirtual(BLYNK_FLOWRATE_SETPOINT);
}
...
void loop()
{
  ...
  float sensorValue = analogRead(ANALOG_INPUT_FLOWRATE);
...
void setup() {
 ...
 pinMode(ANALOG_OUTPUT_AIR_PRESSURE, OUTPUT);
...
void loop() {
 ...
 analogWrite(ANALOG_OUTPUT_AIR_PRESSURE, analogOut0); // set the analog output 0 to 5V or 10mA
